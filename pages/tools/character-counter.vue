  ...
  <div class="tool-content">
    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem; margin-top: 2rem;">
      <div class="result-box">
        <h3 style="margin-bottom: 0.5rem; color: #2563eb;">文字数（スペースあり）</h3>
        <div style="font-size: 1.5rem; font-weight: bold; color: #1e293b;">{{ stats.charactersWithSpaces }}</div>
      </div>
      
      <div class="result-box">
        <h3 style="margin-bottom: 0.5rem; color: #2563eb;">文字数（スペースなし）</h3>
        <div style="font-size: 1.5rem; font-weight: bold; color: #1e293b;">{{ stats.charactersWithoutSpaces }}</div>
      </div>
      
      <div class="result-box">
        <h3 style="margin-bottom: 0.5rem; color: #2563eb;">行数</h3>
        <div style="font-size: 1.5rem; font-weight: bold; color: #1e293b;">{{ stats.lines }}</div>
      </div>
      
      <div class="result-box">
        <h3 style="margin-bottom: 0.5rem; color: #2563eb;">単語数</h3>
        <div style="font-size: 1.5rem; font-weight: bold; color: #1e293b;">{{ stats.words }}</div>
      </div>
      
      <div class="result-box">
        <h3 style="margin-bottom: 0.5rem; color: #2563eb;">段落数</h3>
        <div style="font-size: 1.5rem; font-weight: bold; color: #1e293b;">{{ stats.paragraphs }}</div>
      </div>
      
      <div class="result-box">
        <h3 style="margin-bottom: 0.5rem; color: #2563eb;">バイト数（UTF-8）</h3>
        <div style="font-size: 1.5rem; font-weight: bold; color: #1e293b;">{{ stats.bytes }}</div>
      </div>
    </div>
    ...
    <div style="margin-top: 2rem; padding: 1rem; background: #f8fafc; border-radius: 6px;">
      ...
      <ul style="margin-left: 1.5rem; color: #64748b;">
        <li>上のテキストエリアに文字を入力すると、リアルタイムで統計が更新されます</li>
...
const stats = computed(() => {
  ...
  const symbols = charactersWithoutSpaces - hiragana - katakana - kanji - alphanumeric
...
useHead({
  ...
  meta: [
    { name: 'description', content: 'テキストの文字数、行数、バイト数を瞬時にカウントします。日本語の文字種別分析や読み取り時間の計算も可能です。' },
    { name: 'keywords', content: '文字数, カウント, テキスト, 行数, バイト, 文字種別, 読み取り時間' }
  ]
})
</script>