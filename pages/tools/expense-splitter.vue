  ...
  <div class="tool-content">
      ...
      <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 1rem;">
        ...
        <div class="result-box" style="border-left: 4px solid #10b981;">
          ...
          <div style="background: #f1f5f9; padding: 1rem; border-radius: 6px;">
            <div style="font-size: 0.9rem;">
              ...
              <div>切り上げ差額: ¥{{ (Math.ceil(result.amountPerPerson) - result.amountPerPerson).toFixed(2) }}</div>
            </div>
          </div>
        </div>
        ...
        <div class="result-box">
          ...
          <div style="font-size: 0.9rem; line-height: 1.6;">
            ...
            <div v-if="tipPercentage > 0" style="display: flex; justify-content: space-between; margin-bottom: 0.5rem;">
            ...
            <table style="width: 100%; border-collapse: collapse;">
              <thead>
                <tr style="background: #f8fafc;">
                  <th style="padding: 0.75rem; text-align: left; border: 1px solid #e2e8f0;">参加者</th>
                  <th style="padding: 0.75rem; text-align: right; border: 1px solid #e2e8f0;">支払い金額</th>
                  <th style="padding: 0.75rem; text-align: center; border: 1px solid #e2e8f0;">確認</th>
                </tr>
              </thead>
              <tbody>
                <tr
                  ...
                  :key="person.id"
                  :style="{ backgroundColor: person.paid ? '#f0fdf4' : 'white' }"
                >
                  <td style="padding: 0.75rem; border: 1px solid #e2e8f0;">
                    {{ person.name }}
                  </td>
                  <td style="padding: 0.75rem; text-align: right; border: 1px solid #e2e8f0; font-family: 'Courier New', monospace;">
              ...
              <tfoot>
                <tr style="background: #f1f5f9; font-weight: 600;">
                  ...
                  <td style="padding: 0.75rem; text-align: center; border: 1px solid #e2e8f0;">
                    {{ paidCount === numberOfPeople ? '✓' : `${paidCount}/${numberOfPeople}` }}
                  </td>
                </tr>
              </tfoot>
            </table>
    ...
    <div style="margin-top: 2rem; padding: 1rem; background: #f8fafc; border-radius: 6px;">
      ...
      <ul style="margin-left: 1.5rem; color: #64748b; margin-bottom: 1rem;">
        <li>端数は自動的に切り上げされるため、実際の支払額より少し多めになります</li>
...
const result = computed(() => {
  ...
  return calculateExpenseSplit(totalAmount.value, numberOfPeople.value, tipPercentage.value)
})
...
// 監視
watch([totalAmount, numberOfPeople, tipPercentage], () => {
...
useHead({
  ...
  meta: [
    { name: 'description', content: '複数人での飲み会などの費用を簡単に割り勘計算します。チップやサービス料にも対応。' },
    { name: 'keywords', content: '割り勘計算, 会計, 飲み会, レストラン, 費用分割' }
  ]
})
</script>