  ...
  <div class="tool-content">
    ...
    <div v-if="sequence.length > 0" style="margin-top: 2rem;">
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem;">
        <h3 style="color: #1e293b;">フィボナッチ数列（{{ sequence.length }}項）</h3>
      ...
      <div v-if="displayFormat === 'list'" class="result-box">
        <div style="display: flex; flex-direction: column; gap: 0.5rem; max-height: 400px; overflow-y: auto;">
          ...
          >
            <div>
              <span style="font-weight: 600; color: #374151;">F({{ index }})</span>
              <span style="margin-left: 1rem; font-family: 'Courier New', monospace;">
                {{ number.toLocaleString() }}
              </span>
            </div>
      ...
      <div v-if="displayFormat === 'formula'" class="result-box">
        <div style="font-family: 'Courier New', monospace; line-height: 1.8; word-break: break-all;">
          {{ sequence.map((num, i) => `F(${i}) = ${num.toLocaleString()}`).join('\n') }}
        </div>
      </div>
    </div>
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
          <div>
            ...
            <div style="font-size: 0.9rem;">
              ...
              <div>最大値の桁数: {{ Math.max(...sequence).toString().length }}桁</div>
              <div>総和: {{ sequence.reduce((sum, num) => sum + num, 0).toLocaleString() }}</div>
            </div>
          </div>
          ...
          <div v-if="goldenRatio">
            ...
            <div style="font-size: 0.9rem;">
              ...
              <div>誤差: {{ Math.abs(goldenRatio - 1.6180339887).toFixed(10) }}</div>
              <div>精度: {{ (100 - Math.abs(goldenRatio - 1.6180339887) * 100).toFixed(6) }}%</div>
            </div>
          </div>
    ...
    <div style="margin-top: 2rem; padding: 1rem; background: #f8fafc; border-radius: 6px;">
      <h4 style="color: #1e293b; margin-bottom: 0.5rem;">フィボナッチ数列について</h4>
...
const isValidInput = computed(() => {
  return termCount.value !== null && termCount.value >= 1 && termCount.value <= 1000
})
...
useHead({
  ...
  meta: [
    { name: 'description', content: 'フィボナッチ数列を指定した項数まで生成します。黄金比の確認や統計分析機能付き。' },
    { name: 'keywords', content: 'フィボナッチ数列, 黄金比, 数学, 数列生成, 統計分析' }
  ]
})
</script>