  ...
  <div class="tool-content">
    ...
    <div class="input-section">
      <div class="form-group">
        ...
        />
        <div v-if="ibanNumber" style="margin-top: 0.5rem; font-size: 0.875rem; color: #64748b;">
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
          ...
          <div>
            ...
            <div style="font-size: 0.9rem;">
              ...
              <div>MOD-97チェック: {{ validationResult.mod97Valid ? '✓' : '✗' }}</div>
              <div>国コード: {{ validationResult.countryValid ? '✓' : '✗' }}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="result-box">
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 1rem;">
          <div v-for="sample in sampleIbans" :key="sample.iban">
            <div style="font-weight: 600; margin-bottom: 0.25rem;">{{ sample.country }}</div>
    ...
    <div style="margin-top: 2rem; padding: 1rem; background: #f8fafc; border-radius: 6px;">
      ...
      <ul style="margin-left: 1.5rem; color: #64748b; margin-bottom: 1rem;">
        <li>IBAN（International Bank Account Number）は国際銀行口座番号の標準形式です</li>
        <li>国際送金において口座番号を一意に識別するために使用されます</li>
        <li>MOD-97アルゴリズムによってチェックサムが検証されます</li>
        <li>国コード（2文字）+ チェックデジット（2桁）+ 国内銀行口座識別子で構成されます</li>
      </ul>
...
<script setup>
import { ref } from 'vue'
import { validateIBAN, getIBANCountryCode, formatIBAN as formatIban } from '~/utils/security'
...
useHead({
  ...
  meta: [
    { name: 'description', content: '国際銀行口座番号（IBAN）の妥当性をMOD-97アルゴリズムで検証します。' },
    { name: 'keywords', content: 'IBAN検証, 国際銀行口座番号, MOD-97, 銀行口座検証' }
  ]
})
</script>