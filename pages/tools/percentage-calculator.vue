<template>
  <div class="tool-content">
    ...
    <div style="margin-bottom: 2rem;">
      <div style="display: flex; gap: 0.5rem; flex-wrap: wrap;">
        <button
          ...
          :key="type.id"
          :class="['btn', selectedType === type.id ? 'btn-primary' : 'btn-secondary']"
    ...
    <div v-if="selectedType === 'percentage'" class="calculation-section">
      <h3 style="margin-bottom: 1rem; color: #1e293b;">パーセント計算（A は B の何％？）</h3>
      ...
      <div v-if="percentageResult !== null" class="result-box">
        ...
        <div style="font-size: 1.5rem; font-weight: 600; color: #1e293b;">
          {{ percentageCalc.value }} は {{ percentageCalc.total }} の <span style="color: #10b981;">{{ percentageResult.toFixed(2) }}%</span>
        </div>
      </div>
    </div>
    ...
    <div v-if="selectedType === 'value'" class="calculation-section">
      <h3 style="margin-bottom: 1rem; color: #1e293b;">値計算（A の B％ はいくつ？）</h3>
      <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem; margin-bottom: 1rem;">
        <div class="form-group">
      ...
      <div v-if="valueResult !== null" class="result-box">
        <h4 style="color: #2563eb; margin-bottom: 0.5rem;">結果</h4>
        <div style="font-size: 1.5rem; font-weight: 600; color: #1e293b;">
          {{ valueCalc.total }} の {{ valueCalc.percentage }}% は <span style="color: #10b981;">{{ valueResult.toFixed(2) }}</span>
        </div>
      </div>
    </div>
    ...
    <div v-if="selectedType === 'change'" class="calculation-section">
      <h3 style="margin-bottom: 1rem; color: #1e293b;">増減率計算（A から B への変化率）</h3>
      <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem; margin-bottom: 1rem;">
        <div class="form-group">
    ...
    <div v-if="selectedType === 'increase'" class="calculation-section">
      <h3 style="margin-bottom: 1rem; color: #1e293b;">パーセント増加・減少（A を B％ 増加/減少）</h3>
      <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem; margin-bottom: 1rem;">
        <div class="form-group">
          <label class="form-label">元の値（A）</label>
          <input
      ...
      <div v-if="increaseResult !== null" class="result-box">
        ...
        <div style="margin-top: 0.5rem; font-size: 0.9rem; color: #64748b;">
          変化量: {{ increaseCalc.percentage >= 0 ? '+' : '' }}{{ (increaseResult - increaseCalc.value).toFixed(2) }}
        </div>
      </div>
    </div>
    ...
    <div style="margin-top: 2rem; padding: 1rem; background: #f8fafc; border-radius: 6px;">
      ...
      <div>
        <h5 style="color: #2563eb; margin-bottom: 0.25rem;">パーセント増加・減少</h5>
        <p style="color: #64748b; font-size: 0.875rem; margin-left: 1rem;">
...
const percentageResult = computed(() => {
  if (percentageCalc.value.value !== null && percentageCalc.value.total !== null && percentageCalc.value.total !== 0) {
    return calculatePercentage(percentageCalc.value.value, percentageCalc.value.total)
  }
  return null
})
...
const valueResult = computed(() => {
  if (valueCalc.value.total !== null && valueCalc.value.percentage !== null) {
    return calculateValueFromPercentage(valueCalc.value.percentage, valueCalc.value.total)
  }
  return null
})

const changeResult = computed(() => {
  if (changeCalc.value.oldValue !== null && changeCalc.value.newValue !== null) {
    return calculatePercentageChange(changeCalc.value.oldValue, changeCalc.value.newValue)
  }
  return null
})

const increaseResult = computed(() => {
  if (increaseCalc.value.value !== null && increaseCalc.value.percentage !== null) {
    return calculatePercentageIncrease(increaseCalc.value.value, increaseCalc.value.percentage)
  }
  return null
})
...
useHead({
  ...
  meta: [
    { name: 'description', content: '割合、増減率、パーセンテージを簡単に計算します。パーセント計算、値計算、増減率計算などに対応。' },
    { name: 'keywords', content: 'パーセント計算, 割合計算, 増減率, パーセンテージ, 数学' }
  ]
})
</script>
