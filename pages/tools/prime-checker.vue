  ...
  <div class="tool-content">
      ...
      <div class="result-box">
        <div style="display: flex; align-items: center; gap: 1rem; margin-bottom: 1.5rem;">
          ...
          <div>
            ...
            <div style="font-size: 1.125rem; margin-top: 0.25rem;">
              <span :style="{ color: result.isPrime ? '#10b981' : '#f59e0b', fontWeight: '600' }">
        ...
        <div v-if="!result.isPrime && result.factors.length > 0">
          ...
          <div style="background: #f1f5f9; padding: 1rem; border-radius: 6px; margin-bottom: 1rem;">
            <div style="font-family: 'Courier New', monospace; font-size: 1.125rem; font-weight: 600;">
              {{ inputNumber.toLocaleString() }} = {{ formatFactorization(result.factors) }}
            </div>
          </div>
        </div>
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
          <div>
            ...
            <div style="font-size: 0.9rem;">
              ...
              <div>偶数・奇数: {{ inputNumber % 2 === 0 ? '偶数' : '奇数' }}</div>
              <div v-if="!result.isPrime">約数の個数: {{ result.divisorCount }}個</div>
            </div>
          </div>
    ...
    <div style="margin-top: 2rem; padding: 1rem; background: #f8fafc; border-radius: 6px;">
      ...
      <ul style="margin-left: 1.5rem; color: #64748b; margin-bottom: 1rem;">
        ...
        <li>2は唯一の偶数の素数です（それ以外の偶数は2で割り切れるため素数ではありません）</li>
...
const checkPrime = () => {
  ...
  if (!primeResult) {
    ...
    divisorCount = Object.values(factorCounts).reduce((acc, count) => acc * (count + 1), 1)
  }
...
useHead({
  ...
  meta: [
    { name: 'description', content: '入力した数値が素数かどうかを判定します。素因数分解機能と素数一覧表示も利用できます。' },
    { name: 'keywords', content: '素数判定, 素因数分解, 数学, アルゴリズム, 暗号学' }
  ]
})
</script>