<template>
  <div class="tool-content">
    ...
    <!-- ツールバー -->
    <div style="display: flex; flex-wrap: wrap; gap: 1rem; margin-bottom: 2rem; padding: 1rem; background: #f8fafc; border-radius: 6px;">
      <button
      ...
      <div style="margin-left: auto; display: flex; flex-wrap: wrap; align-items: center; gap: 1rem;">
        <label style="display: flex; align-items: center; gap: 0.5rem;">
          <span style="font-size: 0.875rem; color: #64748b;">方言:</span>
          <select v-model="dialect" class="form-select" style="width: auto; padding: 0.5rem;" @change="onSettingsChange">
        ...
        <label style="display: flex; align-items: center; gap: 0.5rem;">
          ...
          <select v-model="indentSize" class="form-select" style="width: auto; padding: 0.5rem;" @change="onSettingsChange">
        ...
        <label style="display: flex; align-items: center; gap: 0.5rem;">
          ...
          <select v-model="keywordCase" class="form-select" style="width: auto; padding: 0.5rem;" @change="onSettingsChange">
    ...
    <div class="form-group">
      ...
      ></textarea>
      <div v-if="validationErrors.length > 0" style="color: #dc2626; font-size: 0.875rem; margin-top: 0.5rem;">
        <strong>エラー:</strong>
    ...
    <div style="margin-top: 2rem; padding: 1rem; background: #f8fafc; border-radius: 6px;">
      ...
      <ul style="margin-left: 1.5rem; color: #64748b; margin-bottom: 1rem;">
        <li><strong>整形:</strong> SQLクエリを読みやすい形式にインデントを付けて整形します</li>
        <li><strong>圧縮:</strong> 不要な空白や改行を削除してサイズを最小化します</li>
        <li><strong>検証:</strong> SQL構文の基本的な妥当性をチェックしてエラーを表示します</li>
...
import { formatSql, minifySql, validateSql, extractSqlStatistics, detectSqlDialect, type SqlFormatOptions, type SqlStatistics } from '~/utils/sqlFormatter'
...
const dialect = ref<'standard' | 'mysql' | 'postgresql' | 'sqlserver' | 'oracle' | 'sqlite'>('standard')
...
const onSettingsChange = () => {
  if (inputSql.value.trim() && outputSql.value && validationErrors.value.length === 0) {
    formatSqlMethod()
  }
}
...
useHead({
  ...
  meta: [
    { name: 'description', content: 'SQLクエリを見やすく整形・圧縮・検証するオンラインツールです。MySQL、PostgreSQL、SQL Server、Oracle、SQLiteに対応。統計情報の表示やエラー検出も可能です。' },
    { name: 'keywords', content: 'SQL, フォーマッター, 整形, 圧縮, 検証, MySQL, PostgreSQL, Oracle, SQL Server, SQLite' }
  ]
})
</script>
