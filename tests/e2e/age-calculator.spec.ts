...
test.describe('年齢計算ツール', () => {
  ...
  test('ページが正しく表示される', async ({ page }) => {
    ...
    await expect(page.locator('p')).toHaveText('生年月日から現在の年齢を詳細に計算します。')
  ...
  test('詳細な年齢情報の表示', async ({ page }) => {
    ...
    // 別の単位での年齢カード
    await expect(detailCards.nth(1).locator('h3')).toHaveText('別の単位での年齢')
  ...
  test('うるう年の判定', async ({ page }) => {
    ...
    await expect(page.locator('.detail-card').nth(3)).toContainText('うるう年: はい')
    
    // 平年生まれ
    await page.fill('#birth-date', '2001-01-01')
    
    await expect(page.locator('.detail-card').nth(3)).toContainText('うるう年: いいえ')
  })
  ...
  test('人生の主なイベント表示', async ({ page }) => {
    ...
    await expect(lifeEvents.locator('.event-item.completed')).toContainText('誕生')
    await expect(lifeEvents.locator('.event-item.completed')).toContainText('小学校入学')
    await expect(lifeEvents.locator('.event-item.completed')).toContainText('成人（18歳）')
    await expect(lifeEvents.locator('.event-item.completed')).toContainText('三十路')
    
    // まだ到達していないイベントは未完了として表示
    await expect(lifeEvents.locator('.event-item:not(.completed)')).toContainText('四十路')
  })
  ...
  test('未来の日付でエラー表示', async ({ page }) => {
    ...
    await expect(page.locator('.error-message')).toContainText('Birth date cannot be in the future')
  })
  ...
  test('レスポンシブデザイン', async ({ page }) => {
    ...
    const gridColumns = await dateInputs.evaluate(el => 
    ...
    const cardColumns = await detailCards.evaluate(el => 
    ...
    const fontSize = await ageDisplayH2.evaluate(el => 
  ...
  test('ページタイトルとメタデータ', async ({ page }) => {
    ...
    expect(description).toBe('生年月日から現在の年齢を詳細に計算します。総日数、次の誕生日、星座、干支なども表示。')
  })
  ...
  test('詳細な計算精度', async ({ page }) => {
    ...
    await expect(page.locator('.age-display h2')).toContainText('24歳 0ヶ月 0日')
    
    // 非うるう年での計算
    await page.fill('#target-date', '2025-02-28')
    
    await expect(page.locator('.age-display h2')).toContainText('24歳 11ヶ月 30日')
  })
})