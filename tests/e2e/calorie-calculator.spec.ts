...
test.describe('Calorie Calculator', () => {
  ...
  test('should display the tool page correctly', async ({ page }) => {
    ...
    await expect(page.locator('p').first()).toContainText('基礎代謝量（BMR）と1日の消費カロリー（TDEE）を計算')
  })
  ...
  test('should calculate calories for male', async ({ page }) => {
    ...
    await expect(page.locator('.summary-item').nth(0)).toContainText('基礎代謝量（BMR）')
    await expect(page.locator('.summary-item').nth(1)).toContainText('総消費カロリー（TDEE）')
    await expect(page.locator('.summary-item').nth(2)).toContainText('推奨摂取カロリー')
  ...
  test('should calculate calories for female with weight loss goal', async ({ page }) => {
    // Select gender
    ...
    await expect(page.locator('.highlight .description')).toContainText('減量（週0.5kg）')
  })
  ...
  test('should update recommendations based on activity level', async ({ page }) => {
    await page.check('input[value="male"]')
    await page.fill('#age', '30')
    ...
    const sedentaryTDEE = await page.locator('.summary-item').nth(1).locator('.value').textContent()
    
    // Test active
    await page.selectOption('#activityLevel', 'active')
    await page.click('button:has-text("計算する")')
    
    const activeTDEE = await page.locator('.summary-item').nth(1).locator('.value').textContent()
  ...
  test('should show different recommendations for different goals', async ({ page }) => {
    await page.check('input[value="female"]')
    ...
    await page.selectOption('#goal', 'maintain')
    await page.click('button:has-text("計算する")')
    const maintainCalories = await page.locator('.highlight .value').textContent()
