...
test.describe('Compound Interest Calculator', () => {
  ...
  test('should display the tool page correctly', async ({ page }) => {
    ...
    await expect(page.locator('p').first()).toContainText('複利での投資収益や貯蓄額を計算します')
  })

  test('should calculate compound interest without monthly deposits', async ({ page }) => {
  ...
  test('should calculate compound interest with monthly deposits', async ({ page }) => {
    await page.fill('#principal', '1000000')
    await page.fill('#annualRate', '5')
    ...
    const totalDepositsText = await page.locator('.summary-item').nth(1).textContent()
  ...
  test('should handle different compounding frequencies', async ({ page }) => {
    ...
    let futureValueText = await page.locator('.summary-item').nth(0).textContent()
