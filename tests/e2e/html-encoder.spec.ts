...
test.describe('HTML Entity Encoder', () => {
  ...
  test('should decode HTML entities', async ({ page }) => {
    ...
    await decodeTextarea.fill('&lt;p&gt;Hello &amp; &quot;World&quot;&lt;/p&gt;')
  ...
  test('should disable decimal option when non-ASCII is unchecked', async ({ page }) => {
    const decimalCheckbox = page.locator('input[type="checkbox"]').nth(2)
  ...
  test('should display reference table', async ({ page }) => {
    ...
    await expect(headers).toContainText(['文字', 'エンティティ名', '10進数', '16進数', '説明'])
  ...
  test('should display examples', async ({ page }) => {
    ...
    await expect(page.locator('.example-input pre')).toContainText('<p>Hello & "World"</p>')
    await expect(page.locator('.example-output pre')).toContainText('&lt;p&gt;Hello &amp; &quot;World&quot;&lt;/p&gt;')
  })
