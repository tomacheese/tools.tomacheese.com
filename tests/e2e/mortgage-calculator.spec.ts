...
test.describe('Mortgage Calculator', () => {
  ...
  test('has correct title and description', async ({ page }) => {
    ...
    await expect(page.locator('p').first()).toHaveText('住宅ローンの月額返済額、総返済額、利息総額を計算します。年次の返済内訳も確認できます。')
  })
  ...
  test('calculates mortgage with standard inputs', async ({ page }) => {
    ...
    await expect(page.locator('.summary-item').filter({ hasText: '借入金額' }).locator('.value')).toHaveText('¥27,000,000')

    // Verify monthly payment (approximately)
    const monthlyPayment = await page.locator('.summary-item.highlight').filter({ hasText: '月々の返済額' }).locator('.value').textContent()
    expect(monthlyPayment).toMatch(/¥82,\d{3}/)

    // Verify total payment and interest exist
    await expect(page.locator('.summary-item').filter({ hasText: '総返済額' }).locator('.value')).toBeVisible()
    await expect(page.locator('.summary-item').filter({ hasText: '利息総額' }).locator('.value')).toBeVisible()
  ...
  test('calculates mortgage with minimal down payment', async ({ page }) => {
    ...
    await expect(page.locator('.summary-item').filter({ hasText: '借入金額' }).locator('.value')).toHaveText('¥20,000,000')
  ...
  test('calculates mortgage with short term', async ({ page }) => {
    ...
    // Verify results
    await expect(page.locator('.result')).toBeVisible()
    await expect(page.locator('.summary-item').filter({ hasText: '借入金額' }).locator('.value')).toHaveText('¥8,000,000')
  ...
  test('shows additional information', async ({ page }) => {
    ...
    await expect(page.locator('.additional-info').locator('.label')).toHaveText('返済負担率')
  ...
  test('calculates with zero interest rate', async ({ page }) => {
    ...
    await expect(page.locator('.summary-item').filter({ hasText: '借入金額' }).locator('.value')).toHaveText('¥10,000,000')
    await expect(page.locator('.summary-item').filter({ hasText: '利息総額' }).locator('.value')).toHaveText('¥0')
  })
