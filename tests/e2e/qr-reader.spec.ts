...
test.describe('QRコード読み取りツール', () => {
  ...
  test('ページが正しく表示される', async ({ page }) => {
    ...
    await expect(page.locator('p').first()).toContainText('画像からQRコードを読み取り、テキストを抽出します。')
  })

  test('ファイルアップロード領域が表示される', async ({ page }) => {
    await expect(page.locator('.upload-area')).toBeVisible()
    await expect(page.locator('.upload-content')).toContainText('QRコード画像をアップロード')
  ...
  test('カメラセクションが表示される', async ({ page }) => {
    ...
    await expect(page.locator('h3').filter({ hasText: 'またはカメラで撮影' })).toBeVisible()
    await expect(page.locator('button').filter({ hasText: 'カメラを開始' })).toBeVisible()
  })

  test('使用方法セクションが表示される', async ({ page }) => {
    await expect(page.locator('.help-section')).toBeVisible()
    await expect(page.locator('h3').filter({ hasText: '使用方法' })).toBeVisible()
    await expect(page.locator('li').first()).toContainText('QRコードが含まれた画像をアップロードするか、カメラで撮影してください')
  })
  ...
  test('アップロード後に画像プレビューが表示される', async ({ page }) => {
    ...
    await expect(page.locator('button').filter({ hasText: '画像をクリア' })).toBeVisible()
  })
  ...
  test('解析ボタンが画像アップロード後に表示される', async ({ page }) => {
    ...
    await expect(page.locator('.analyze-button')).toContainText('QRコードを解析')
  })
  ...
  test('キーボードナビゲーションが可能', async ({ page }) => {
    ...
    // カメラボタンにフォーカス
    await page.keyboard.press('Tab')
    await expect(page.locator('button').filter({ hasText: 'カメラを開始' })).toBeFocused()
  })

  test('画像形式の説明が表示される', async ({ page }) => {
    await expect(page.locator('.upload-hint')).toContainText('JPG、PNG、WebP形式に対応')
  })

  test('プライバシーに関する説明が表示される', async ({ page }) => {
    await expect(page.locator('.help-section li').last()).toContainText('すべての処理はブラウザ内で行われ、画像は外部に送信されません')
  })
})