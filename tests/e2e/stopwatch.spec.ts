...
test.describe('ストップウォッチツール', () => {
  ...
  test('ページが正しく表示される', async ({ page }) => {
    ...
    await expect(page.locator('p')).toHaveText('高精度なストップウォッチツールです。ラップタイムの記録や統計表示が可能。')
    
    // 初期状態の確認
    await expect(page.locator('.main-time')).toHaveText('00:00.00')
    await expect(page.locator('.control-button.primary')).toHaveText('開始')
    await expect(page.locator('.control-button.secondary')).toHaveText('リセット')
  ...
  test('基本的なストップウォッチ操作', async ({ page }) => {
    ...
    // 停止後の状態確認
    await expect(page.locator('.control-button.primary')).toHaveText('開始')
    await expect(page.locator('.control-button.primary')).not.toHaveClass(/running/)
    await expect(page.locator('.control-button.secondary')).toHaveText('リセット')
  })
  ...
  test('データエクスポート機能', async ({ page }) => {
    ...
    expect(download.suggestedFilename()).toMatch(/^stopwatch-data-\d{4}-\d{2}-\d{2}\.json$/)
  })

  test('ラップなし状態の表示', async ({ page }) => {
    // 初期状態でラップなしのメッセージが表示される
    await expect(page.locator('.no-laps')).toBeVisible()
    await expect(page.locator('.no-laps-content h3')).toHaveText('ラップタイムなし')
    await expect(page.locator('.no-laps-content p')).toContainText('ストップウォッチを開始してラップボタンを押すと、ラップタイムが記録されます。')
  })
  ...
  test('レスポンシブデザイン', async ({ page }) => {
    ...
    const fontSize = await mainTime.evaluate(el => 
  ...
  test('キーボードショートカット説明の表示', async ({ page }) => {
    ...
    await expect(page.locator('.keyboard-shortcuts h3')).toHaveText('キーボードショートカット')
  ...
  test('ページタイトルとメタデータ', async ({ page }) => {
    ...
    expect(description).toBe('高精度なストップウォッチツール。ラップタイムの記録、統計表示、データエクスポート機能付き。')
  })
