...
test.describe('Timestamp Converter', () => {
  ...
  test('should display the timestamp converter page', async ({ page }) => {
    ...
    await expect(page.locator('h2')).toContainText(['現在時刻', 'タイムスタンプ・日時変換', '日時からタイムスタンプ生成'])
  })

  test('should display current time and update it', async ({ page }) => {
    // Get initial Unix timestamp
    const initialTimestamp = await page.locator('.current-time .time-info').first().locator('code').textContent()
    
    // Wait for 2 seconds
    await page.waitForTimeout(2000)
    
    // Get updated Unix timestamp
    const updatedTimestamp = await page.locator('.current-time .time-info').first().locator('code').textContent()
  ...
  test('should convert Unix timestamp (seconds) to date', async ({ page }) => {
    ...
    await expect(page.locator('.time-details .time-info').filter({ hasText: 'Unix タイムスタンプ (秒)' }).locator('code')).toHaveText('1704067200')
    await expect(page.locator('.time-details .time-info').filter({ hasText: 'Unix タイムスタンプ (ミリ秒)' }).locator('code')).toHaveText('1704067200000')
    await expect(page.locator('.time-details .time-info').filter({ hasText: 'ISO 8601' }).locator('code')).toHaveText('2024-01-01T00:00:00.000Z')
  })

  test('should convert Unix timestamp (milliseconds) to date', async ({ page }) => {
  ...
  test('should copy Unix timestamp to clipboard', async ({ page, context }) => {
    ...
    await page.locator('.time-details .time-info').first().locator('button').click()
  })

  test('should convert date picker selection to timestamp', async ({ page }) => {
    // Set date using date picker
    await page.fill('#date-picker', '2024-01-01T00:00')
    
    // Check the result
    await expect(page.locator('.section').nth(2).locator('.result')).toBeVisible()
    await expect(page.locator('.section').nth(2).locator('.time-info').filter({ hasText: 'Unix タイムスタンプ (秒)' }).locator('code')).toContainText('170406')
  })
  ...
  test('should handle negative timestamps', async ({ page }) => {
    ...
    await page.waitForSelector('.result')
    await expect(page.locator('.time-details .time-info').filter({ hasText: 'Unix タイムスタンプ (秒)' }).locator('code')).toHaveText('-1000')
  })
  ...
  test('should handle floating point timestamps', async ({ page }) => {
    ...
    await page.waitForSelector('.result')
    await expect(page.locator('.time-details .time-info').filter({ hasText: 'Unix タイムスタンプ (ミリ秒)' }).locator('code')).toHaveText('1704067200123')
  })
  ...
  test('should show relative time', async ({ page }) => {
    ...
    await page.waitForSelector('.result')
    await expect(page.locator('.time-details .time-info').filter({ hasText: '相対時間' }).locator('code')).toContainText('1日前')
  })
