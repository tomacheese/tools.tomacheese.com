...
test.describe('UUID Generator', () => {
  ...
  test('should generate a single UUID with default options', async ({
    page,
  }) => {
  ...
  test('should regenerate UUIDs', async ({ page }) => {
    // Generate first set
    await page.click('button:has-text("UUID生成")')
    const firstUuid = await page
      .locator('.uuid-item code')
      .first()
      .textContent()

    // Generate second set
    await page.click('button:has-text("UUID生成")')
    const secondUuid = await page
      .locator('.uuid-item code')
      .first()
      .textContent()
