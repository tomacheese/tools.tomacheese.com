...
test.describe('World Clock Tool', () => {
  ...
  test('should display world clock tool correctly', async ({ page }) => {
    ...
    await expect(page.locator('.tool-header p')).toContainText('世界各地の現在時刻を表示します')

    // Check search input
    await expect(page.locator('#citySearch')).toBeVisible()
    await expect(page.locator('label[for="citySearch"]')).toHaveText('都市を検索')
  ...
  test('should display default cities', async ({ page }) => {
    ...
    await expect(page.locator('.city-card:has-text("ニューヨーク")')).toBeVisible()
  ...
  test('should update time every second', async ({ page }) => {
    const timeDisplay = page.locator('.city-card:has-text("東京") .time-display')
  ...
  test('should search for cities', async ({ page }) => {
    ...
    await expect(suggestions.locator('.suggestion-item')).toContainText('バンコク')
  })
  ...
  test('should remove custom city', async ({ page }) => {
    ...
    await expect(page.locator('.city-card:has-text("バンコク")')).not.toBeVisible()
  ...
  test('should reset to defaults', async ({ page }) => {
    ...
    const resetButton = page.locator('button:has-text("デフォルト都市にリセット")')
  ...
  test('should show correct timezone offsets', async ({ page }) => {
    ...
    await expect(page.locator('.city-card:has-text("東京") .timezone-info')).toContainText('UTC+9:00')
    await expect(page.locator('.city-card:has-text("ロンドン") .timezone-info')).toContainText(/UTC[+]\d:00/)
    await expect(page.locator('.city-card:has-text("ニューヨーク") .timezone-info')).toContainText(/UTC[-]\d:00/)
  })

  test('should maintain state when toggling format multiple times', async ({ page }) => {
    const toggle = page.locator('input[type="checkbox"]')
    const timeDisplay = page.locator('.city-card:has-text("東京") .time-display')
  ...
  test('should show usage instructions', async ({ page }) => {
    ...
    await expect(page.locator('li')).toContainText(['検索ボックスに都市名を入力'])
    await expect(page.locator('li')).toContainText(['12時間形式と24時間形式を切り替え'])
  ...
  test('should have proper sidebar navigation', async ({ page }) => {
    ...
    await expect(page.locator('.sidebar-nav a:has-text("世界時計")')).toBeVisible()
  })
})