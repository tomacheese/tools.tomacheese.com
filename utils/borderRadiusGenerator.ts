...
export function generateCSSCode(borderRadius: string, selector: string = '.box'): string {
  return `${selector} {
...
export function parseBorderRadius(cssValue: string): BorderRadiusConfig {
  ...
  if (cleanValue.includes('/')) {
    ...
    if (values.length === 1) {
      ...
      config.topRight.horizontal = config.topRight.vertical = values[1] || values[0] || 0
      config.bottomRight.horizontal = config.bottomRight.vertical = values[2] || values[0] || 0
      config.bottomLeft.horizontal = config.bottomLeft.vertical = values[3] || values[1] || values[0] || 0
    }
  }
...
export function getShapeFromConfig(config: BorderRadiusConfig): string {
  ...
  if (
    unit === '%' &&
    topLeft.horizontal === 50 && topLeft.vertical === 50 &&
    topRight.horizontal === 50 && topRight.vertical === 50 &&
    bottomRight.horizontal === 50 && bottomRight.vertical === 50 &&
    bottomLeft.horizontal === 50 && bottomLeft.vertical === 50
  ) {
    return 'circle'
  }
  ...
  // Check for pill
  if (
    topLeft.horizontal > 1000 && topLeft.vertical > 1000 &&
    topRight.horizontal > 1000 && topRight.vertical > 1000 &&
    bottomRight.horizontal > 1000 && bottomRight.vertical > 1000 &&
    bottomLeft.horizontal > 1000 && bottomLeft.vertical > 1000
  ) {
    return 'pill'
  }
