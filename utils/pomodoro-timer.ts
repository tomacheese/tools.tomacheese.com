...
export function playNotificationSound(pattern: 'complete' | 'warning' = 'complete'): void {
  if (!audioContext) {
    initializeAudio()
  }
  ...
  oscillator.stop(audioContext.currentTime + (pattern === 'complete' ? 0.5 : 0.2))
}
...
export function getDailySummary(sessions: SessionRecord[]): {
  ...
  const completedWork = todaySessions.filter(s => s.type === 'work' && s.completed)
  const completedBreaks = todaySessions.filter(s => s.type === 'break' && s.completed)
...
export function handleKeyboardShortcut(
  ...
  // R to reset
  if (event.code === 'KeyR' && !event.ctrlKey && !event.metaKey && handlers.resetTimer) {
    event.preventDefault()
    handlers.resetTimer()
    return true
  }
  ...
  // S to skip
  if (event.code === 'KeyS' && !event.ctrlKey && !event.metaKey && handlers.skipPhase) {
    event.preventDefault()
    handlers.skipPhase()
    return true
  }
