 ...
 */
export function getDefaultUserAgents(): {
  name: string
  description: string
}[] {
  return [
...
export function getCommonPaths(): {
  ...
  return [
    ...
    { path: '/wp-admin/', description: 'WordPress管理画面', type: 'disallow' },
    {
      ...
      description: 'WordPressアップロード',
      type: 'allow',
    },
    {
      path: '/wp-includes/',
      description: 'WordPressファイル',
      type: 'disallow',
    },
 ...
 */
export function getRobotsTxtTemplates(): {
  name: string
  description: string
 ...
 * robots.txt の基本的な検証
 */
export function validateRobotsTxt(config: RobotsTxtConfig): {
  isValid: boolean
  errors: string[]
} {
  const errors: string[] = []
  ...
  config.userAgentRules.forEach((rule, index) => {
    if (!validateUserAgent(rule.userAgent)) {
      errors.push(
        `ルール${index + 1}: User-Agent「${rule.userAgent}」が無効です`
      )
    }
    ...
    if (
      ...
      (rule.crawlDelay < 0 || !Number.isInteger(rule.crawlDelay))
    ) {
      errors.push(
        `ルール${index + 1}: Crawl-delay「${rule.crawlDelay}」が無効です（正の整数である必要があります）`
      )
    }
  })
...
export function generatePreviewInfo(config: RobotsTxtConfig): {
  ...
  const hasWildcardRule = config.userAgentRules.some(
    rule => rule.userAgent === '*'
  )
  const restrictiveRules = config.userAgentRules.filter(
    rule => rule.disallow.length > 0 || rule.crawlDelay !== undefined
  ).length

  const estimatedSize =
    fileSize < 1024 ? `${fileSize} bytes` : `${(fileSize / 1024).toFixed(1)} KB`
