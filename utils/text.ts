...
export const analyzeText = (text: string): TextStats => {
  ...
  const symbols = Math.max(0, charactersWithoutSpaces - hiragana - katakana - kanji - alphanumeric)
 ...
 * JSONの妥当性をチェックし、解析する
 */
export const parseJsonSafely = (jsonString: string): { success: boolean; data?: any; error?: string } => {
  try {
 ...
 * JSONを整形する
 */
export const formatJson = (jsonString: string, indent: string | number = 2): string => {
 ...
 * テキストの大文字・小文字変換
 */
export const convertCase = (text: string, caseType: 'upper' | 'lower' | 'title' | 'camel' | 'pascal' | 'snake' | 'kebab'): string => {
  switch (caseType) {
    case 'upper':
 ...
 */
export const generateLoremIpsum = (paragraphs: number = 1, startWithLorem: boolean = true): string => {
  const loremWords = [
...
export const decodeHtmlEntities = (text: string): string => {
  ...
  return text.replace(/&(amp|lt|gt|quot|#39|#x2F);/g, (entity) => entityMap[entity])
}

/**
 * テキストから行の差分を計算する
 */
export const calculateTextDiff = (text1: string, text2: string): Array<{ type: 'added' | 'removed' | 'unchanged'; content: string }> => {
  ...
  while (i < lines1.length && j < lines2.length) {
    if (lines1[i] === lines2[j]) {
      ...
      if (line1InText2 !== -1 && (line2InText1 === -1 || line1InText2 - j < line2InText1 - i)) {
