...
export const unitDefinitions: Record<UnitCategory, Record<string, UnitDefinition>> = {
  ...
  area: {
    ...
    squareKilometer: { name: '平方キロメートル', symbol: 'km²', toBase: 1000000 },
    squareCentimeter: { name: '平方センチメートル', symbol: 'cm²', toBase: 0.0001 },
  ...
  speed: {
    ...
    kilometerPerHour: { name: 'キロメートル毎時', symbol: 'km/h', toBase: 0.277778 },
...
export function convertUnit(value: number, fromUnit: string, toUnit: string, category: UnitCategory): number {
  if (category === 'temperature') {
    ...
    if (fromUnit === 'celsius' && toUnit === 'fahrenheit') return celsiusToFahrenheit(value)
    if (fromUnit === 'fahrenheit' && toUnit === 'celsius') return fahrenheitToCelsius(value)
    if (fromUnit === 'celsius' && toUnit === 'kelvin') return celsiusToKelvin(value)
    if (fromUnit === 'kelvin' && toUnit === 'celsius') return kelvinToCelsius(value)
    if (fromUnit === 'fahrenheit' && toUnit === 'kelvin') return fahrenheitToKelvin(value)
    if (fromUnit === 'kelvin' && toUnit === 'fahrenheit') return kelvinToFahrenheit(value)
...
export function getUnitDefinition(unit: string, category: UnitCategory): UnitDefinition | undefined {
